...
const showRecipe = (async function() {
    try {
        ...
        if (!id) return renderError("No recipe ID found in the URL.");
        renderSpinner(recipeContainer);
        // 1) Loading recipe
        await _modelJs.loadRecipe(id);
        const { recipe } = _modelJs.state;
        console.log("Loaded Recipe:", recipe); // Debugging
        if (!recipe) return renderError("Recipe not found."); // Handle case if recipe is not found
        // 2) Rendering recipe
        const markup = `
        ...
        console.error("Error loading recipe:", err); // Better error logging
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["hycaY","aenu9"], "aenu9", "parcelRequire0005")
